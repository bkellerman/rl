...
class DQNMountainCarSolver():
    def __init__(self, n_episodes=1000, gamma=0.99, epsilon=1.0, epsilon_min=0.01,
                 epsilon_decay=0.9995, alpha=0.01, alpha_decay=0.99995, alpha_min=0.000001,
                 batch_size=256, online_update_freq=1, target_update_freq=5, quiet=False):

        self.memory = deque(maxlen=1000000)
        self.pos_memory = deque(maxlen=1000000)
        ...
        self.env = gym.wrappers.Monitor(env, 'mountaincar_rw1', video_callable=EVERY_100, force=True)
    ...
    def replay(self, batch_size):
        ...
        pos_minibatch = random.sample(
            self.pos_memory, min(len(self.pos_memory), batch_size))
        ...
        self.online_model.fit(x_batch, y_batch, batch_size=len(x_batch), epochs=1, verbose=0, shuffle=True)
